...
public class test3 {
    ...
    public static  int delete(String id)
    {
        ...
        try {
            ...
            String sql = "delete from users where usersID"+"="+ i;
    ...
    public static  int insert(String[] str)
    {
        ...
        try {
            ...
            final String DB_URL = "jdbc:mysql://localhost:3306/mydb?useUnicode=true&characterEncoding=UTF-8&useSSL=false&serverTimezone=UTC";
            conn = (Connection)DriverManager.getConnection(DB_URL,"root","root");
            String sql = "insert into users(UsersID, UserCode, UserName, UserInfor) values("+i+",'"+str[1]+"','"+str[2]+"','"+str[3]+"')";
            stmt = conn.createStatement();
            int a = stmt.executeUpdate(sql);
            if(a>0)
                loginsuccess = 1;
    ...
    public static  int reset(String id)
    {
        ...
        Statement stmt = null;
        int i = -1;
        int mima = (int)Math.random()*1000000+1;
        try{
            i = Integer.parseInt(id);
        }
        catch(Exception e1) {
        ...
        try {
            ...
            final String DB_URL = "jdbc:mysql://localhost:3306/mydb?useUnicode=true&characterEncoding=UTF-8&useSSL=false&serverTimezone=UTC";
            conn = (Connection)DriverManager.getConnection(DB_URL,"root","root");
            String sql = "update users set UserCode = md5("+mima+") where usersID"+"="+ i;
            stmt = conn.createStatement();
            int a = stmt.executeUpdate(sql);
            if(a>0)
                loginsuccess = 1;
